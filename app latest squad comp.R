...
#' notes: increase size of legend comet plot 
#' increase size of points comet plot. 
#' find appropriate colour palette for both background colours. use metbrewer itself. 
        ...
        if (input$plot_type == "Squad Table") {
          ...
          theme_custom <- function() {
            theme_minimal() +
              ...
              theme(plot.title = element_text(colour = colorText, size = 28, face = "bold", hjust = 0.5),
                    ...
                    plot.caption = element_text(colour = colorText, size = 8, hjust = 1),
                    axis.title.x = element_text(colour = colorText, size = 14),
                    axis.title.y = element_blank(),
                    axis.text.x = element_text(colour = colorText, size = 10),
                    axis.text.y = element_text(colour = colorText, size = 10)) +
          ...
          ggplot(df) +
            geom_link(aes(x = age_when_joined, y = player_name, xend = age, yend = player_name, 
                          alpha = stat(index), colour = Position), size = 3, show.legend = FALSE) +
            geom_point(aes(x = age, y = player_name, fill = Position), size = 3, shape = 21, stroke = 1) +
            ...
            scale_x_continuous(breaks = c(18:38), labels = c(18:38)) +
            ...
            labs(title = glue("{df$squad} Squad Composition"),
                 ...
                 x = "Age", 
                 caption = "Comet Tails correspond to age at which players joined the club\nComet Points correspond to the player's current age\nDiamonds represent the age at which their contract expires\nCreated by Harsh Krishna")) +
            theme_custom()
